...
namespace Atomic.Entities
{
    public partial class EntityWorld
    {
        ...
        {
            foreach (int key in entity.Values.Keys)
                if (this._tags.TryGetValue(key, out List<IEntity> entities))
                    entities.Remove(entity);

            this.OnStateChanged?.Invoke();
        }

        private void OnValueAdded(IEntity entity, int valueKey, object _)
        {
            if (!_values.TryGetValue(valueKey, out List<IEntity> entities))
            {
                entities = new List<IEntity>();
        ...
        private void OnValueRemoved(IEntity entity, int valueKey, object _)
        {
            if (_values.TryGetValue(valueKey, out List<IEntity> entities))
            {
                entities.Remove(entity);
